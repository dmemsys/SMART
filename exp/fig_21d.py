...
cache_size                = 0
node_type_nums            = fig_params['node_type_num']
...
def main(cmd: CMDManager, tp: LogParser):
    ...
    for node_type_num in node_type_nums:
        # change config
        sed_cmd = generate_sed_cmd('./include/Common.h', method == 'Sherman', 8 if key_type == 'randint' else 32, value_size, cache_size, MN_num)
        cmake_option = cmake_options[method].replace('-DENABLE_CACHE=on', '-DENABLE_CACHE=off')
        # change node type num
        if node_type_num == 1:
            cmake_option = cmake_option.replace('-DENABLE_ADAPTIVE_NODE=on', '-DENABLE_ADAPTIVE_NODE=off')
        else:
            cmake_option = cmake_option.replace('-DTEST_NODE_TYPE_NUM=off', '-DTEST_NODE_TYPE_NUM=on')
            sed_cmd += f"&& {sed_node_type_num('./include/Common.h', node_type_num)}"
        ...
        print_GOOD(f"[FINISHED POINT] method={method} node_type_num={node_type_num} tpt={tpt}")
