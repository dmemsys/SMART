...
from utils.sed_generator import generate_sed_cmd, sed_node_type_num
...
local_lock_nums           = fig_params['local_lock_num']
...
def main(cmd: CMDManager, tp: LogParser):
    ...
    for local_lock_num in local_lock_nums:
        # change config
        sed_cmd = generate_sed_cmd('./include/Common.h', method == 'Sherman', 8 if key_type == 'randint' else 32, value_size, cache_size, MN_num, None, local_lock_num if local_lock_num > 0 else 4)
        cmake_option = cmake_options[method]
        # change local_lock_num
        if local_lock_num == 0:
            cmake_option = cmake_option.replace('-DREAD_DELEGATION=on', '-DREAD_DELEGATION=off')
            cmake_option = cmake_option.replace('-DWRITE_COMBINING=on', '-DWRITE_COMBINING=off').replace('-DDISABLE_HOCL=off', '-DDISABLE_HOCL=on')
        ...
        print_GOOD(f"[FINISHED POINT] method={method} local_lock_num={local_lock_num} tpt={tpt}")
